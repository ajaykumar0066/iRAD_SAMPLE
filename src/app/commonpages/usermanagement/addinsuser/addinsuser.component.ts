...
@Component({
  selector: 'app-addinsuser',
  templateUrl: './addinsuser.component.html',
  styleUrls: ['./addinsuser.component.scss'],
})
export class AddinsuserComponent implements OnInit {
  ...
  saveModal() {
    this.message = null;
    console.log(this.insuserdata);
    let postDate = {
      mode: 'addINSUser',
      admin: this.admin,
      user: this.insuserdata
    }
  ...
  validationCheck() {
    var mobile = new String(this.insuserdata.mobile);
    let isnum = /^\d+$/.test(this.insuserdata.mobile);
    var email = new String(this.insuserdata.email);

    var nameval = new String(this.insuserdata.name);
    var loginid = new String(this.insuserdata.login_id);


    var name;
    if (this.insuserdata.login_id.match("^[A-Za-z0-9]+$")) {
      console.log("true")
      name = true;
    } else {
      console.log("false")
    ...
    if (this.insuserdata.name != '' && this.insuserdata.login_id != '' && this.insuserdata.mobile != '') {
      if (/\s/.test(this.insuserdata.email) && /\s/.test(this.insuserdata.mobile) && /\s/.test(this.insuserdata.login_id)) {
