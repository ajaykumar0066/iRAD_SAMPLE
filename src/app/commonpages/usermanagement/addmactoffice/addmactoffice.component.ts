  ...
  @Component({
    selector: 'app-addmactoffice',
    templateUrl: './addmactoffice.component.html',
    styleUrls: ['./addmactoffice.component.scss'],
  })
  export class AddmactofficeComponent implements OnInit {

    @Input() insdata:any;
    mact={'name':'','address':'','mobile':'','email':''};
    ...
    validationCheck(){
      ...
      //this.rto.uo_code=check;
            ...
            if (this.mact.name.length >= 5 ) {
              if(this.mact.address.length >= 10){
                if(this.mact.mobile.length >= 10){
                  if(this.mact.email.length >= 5){
                    this.saveModal();
                  }
                  else{
