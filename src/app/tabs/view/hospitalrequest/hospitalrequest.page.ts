...
@Component({
  selector: 'app-hospitalrequest',
  templateUrl: './hospitalrequest.page.html',
  styleUrls: ['./hospitalrequest.page.scss'],
})
export class HospitalrequestPage implements OnInit {
  ...
  postdata3={'mode':'','language':'','name':'','pid':''};
  ...
  async showmapping(id) 
  {
   
    const modal = await this.modalctrl.create({
      component: AccmappingComponent,
      componentProps: {
        'pid':this.comp_data[id].id,
        'hid':this.comp_data[id].hospital_id,
        'pname':this.comp_data[id].patient_name,
        'gender':this.comp_data[id].gender,
        'address':this.comp_data[id].patient_address,
        'hpname':this.comp_data[id].hpname,
        'hpaddr':this.comp_data[id].hpaddr,
        'timeof_arrival':this.comp_data[id].timeof_arrival,
      }
    });
...
getRows() {
 ...
 `, [])
  ...
  funPending(offset){
    ...
    this.api.post('hprequesttlist.php',postDate).subscribe((data: any)=>{
      this.loading=false;
      if(offset==0){ 
  ...
  viewAccidentPDF(i){
   ...
   let pdfurl=environment.apiUrl+'reports/health_plreqquest.php?ln='+this.ln+'&pid='+i;
  ...
  viewddcertificate(i){
   // console.log('sdsdfsdfds:  '+this.comp_data);
    //alert('afdfsdf');
    // this.router.navigate(['/accview']);
    //hospitalrequest
   // let pdfurl=environment.apiUrl+'reports/health_dischargecertificate.php';
    //let pdfurl=environment.apiUrl+'reports/health_ddcertificate.php?ln='+this.ln+'&pid='+i;
    let pdfurl = environment.apiUrl + 'healthdocumentview.php?pid=' +i+'&docname=drunken';
   ...
   viewdischagre(i){
    // console.log('sdsdfsdfds:  '+this.comp_data);
     ...
     //hospitalrequest
    // let pdfurl=environment.apiUrl+'reports/health_dischargecertificate.php';
     //let pdfurl=environment.apiUrl+'reports/health_dischargecertificate.php?ln='+this.ln+'&pid='+i;
     let pdfurl = environment.apiUrl + 'healthdocumentview.php?pid=' +i+'&docname=discharge';
   ...
   viewpmcertifcate(i){
    // console.log('sdsdfsdfds:  '+this.comp_data);
     ...
     //hospitalrequest
    // let pdfurl=environment.apiUrl+'reports/health_dischargecertificate.php';
    // let pdfurl=environment.apiUrl+'reports/health_pmcertificate.php?ln='+this.ln+'&pid='+i;
     let pdfurl = environment.apiUrl + 'healthdocumentview.php?pid=' +i+'&docname=postmorterm';
  ...
  public requestdischarge(i)
  {

    this.postdata3.mode='dcrequesttohospital';
    this.postdata3.pid=i;
      
        this.giverequest(this.postdata3).subscribe(
          (success:any) => {
  ...
  public giverequest(postdata2:any){
